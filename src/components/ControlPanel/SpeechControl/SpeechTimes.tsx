...
export function SpeechTimes() {
  const { dados, PatchSpeechParl, GetIdSpeech, getIdSpeech } =
    useContext(AuthContext);
  const [timerOn, setTimerOn] = useState(false);
  const [totalTimeInSeconds, setTotalTimeInSeconds] = useState(10 * 60); //dados?.data?.speechParl?.speechTimeInit)
  ...
  return (
    <div className="flex w-full justify-between">
      <div className="flex flex-col">
        <span className="font-bold">Cronometro do Discurso</span>
        <div className="my-2">
        ...
        <div className="flex gap-4">
          {timerOn === false ? (
            <span
              onClick={handleInitTimerSpeech}
              className="flex cursor-pointer border px-4 rounded bg-green-200 hover:bg-green-400"
            >
              iniciar
            </span>
          ) : (
            <span
              onClick={handleInitTimerSpeechNot}
              className="flex cursor-pointer border px-4 rounded bg-green-200 hover:bg-green-400"
            >
              pausar
            </span>
